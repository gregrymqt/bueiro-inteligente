...
namespace backend.Features.Uploads.Infrastructure.Persistence.Configuration;
...
public class UploadModelConfiguration : IEntityTypeConfiguration<UploadModel>
{
    public void Configure(EntityTypeBuilder<UploadModel> builder)
    {
        ...
        builder.Property(x => x.DeviceType).HasColumnName("device_type").IsRequired().HasConversion<string>();
        ...
        builder.HasData(
            new UploadModel
            {
                ...
                DeviceType = backend.Features.Uploads.Domain.Enums.DeviceType.Desktop
            }
        );
    }
}
